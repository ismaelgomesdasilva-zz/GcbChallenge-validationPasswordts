...
const iChars: string = "!`@#$%^&*()+=-[]\\';,./{}|\":<>?~_";
const MIN_LENGTH_PASSWORD: number = 16;
const MAX_LENGTH_PASSWORD: number = 32;
//Variaveis usadas para verificações
...
function isValidSpecialCharacther(password : string):boolean {
...
function isValidSequentialLetter(password : string):boolean{
    ...
    for (let l = 0; l < password.length; l++) {
        ...
        if (
            passwordUpper.charCodeAt(l + 1) == passwordUpper.charCodeAt(l) + 1 &&
...
function isValidSequentialNumber(password : string):boolean{
    for (let j = 0; j < password.length; j++) {
        ...
        if (
            +passwordLower[j + 1] == +passwordLower[j] + 1 &&
...
export function isValidCheck(password : string):Response {
    let resultErros: Response = {
        result: true,
        error: [],
    };
